...
export default function LoginPage() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      if (d.role !== 'BUYER') { setErr('Use the correct portal for your role.'); setLoading(false); return }
      setAuth(d); nav('/stores')
  ...
  return (
    <div className="min-vh-100 d-flex align-items-center justify-content-center">
      <div className="card shadow p-4" style={{minWidth:340}}>
        <h5 className="text-center mb-1">🍔 Customer Login</h5>
        <p className="text-center text-muted small mb-3">localhost:5173</p>
        ...
        <form onSubmit={submit}>
          ...
          <button className="btn btn-danger w-100" disabled={loading}>{loading?'..':'Login'}</button>
        </form>
        <p className="text-center mt-3 small">No account? <Link to="/register">Register</Link></p>
        <hr/><small className="text-muted">Seller: localhost:5174 | Admin: localhost:5175</small>
      </div>
    </div>
  )
}
