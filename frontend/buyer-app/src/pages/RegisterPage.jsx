...
export default function RegisterPage() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      const d = res.data?.data; if(!d?.token) throw new Error('Failed')
  ...
  return (
    <div className="min-vh-100 d-flex align-items-center justify-content-center">
      <div className="card shadow p-4" style={{minWidth:360}}>
        <h5 className="text-center mb-3">🍔 Customer Registration</h5>
          ...
          {[['fullName','Full Name'],['email','Email'],['password','Password'],['phone','Phone'],['address','Address']].map(([k,l])=>(
            <div className="mb-2" key={k}><label className="form-label">{l}</label>
              <input className="form-control" type={k==='email'?'email':k==='password'?'password':'text'}
                value={form[k]} onChange={e=>setForm({...form,[k]:e.target.value})} required={['fullName','email','password'].includes(k)}/>
            </div>
          ))}
          <button className="btn btn-danger w-100 mt-2" disabled={loading}>{loading?'...':'Register as Customer'}</button>
        </form>
